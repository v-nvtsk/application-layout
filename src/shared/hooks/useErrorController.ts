...
/**
 * Хук нормализации и управления ошибками API.
 * Маппит сырую ошибку API в AppError и выставляет isCritical
 * по статусу (>= 500 или === 403).
 */
export function useErrorController() {
  const [error, setError] = useState<AppError | null>(null);

  const handleApiError = useCallback(
    (err: RawApiError | null | undefined, forceCritical = false) => {
      if (!err) {
        setError(null);
        return;
      }
